...
Rowy calls the provided endpoint with a `POST` request, passing the row data in the request body, and an optional query search query `q`, when the users types any text in the search box
