...
to the table. By default, auditing is enabled.
...
The **Auditing options** for a table can be enabled while creating the table. The **Auditing** 
tab allows you to set which fields will be audited when a user updates a table.
Rowy provides an audit trail for each table. The audit trail is a log of all changes made to the table.
...
If enabled earlier, the **Audit Options** can be disabled later by accessing the **Table Settings** on 
the top right corner of the table. Likewise, they can be enabled in the same manner if they were 
disabled earlier.
