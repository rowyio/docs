...
The `db` parameter can access your Firestore to access any collection or document. The above line  above would find the collection named `touristAreas` in your Firestore Database, and try to search for a document with its **Document-ID** matching the value in the field `touristArea` in your current collection. This would return the region field of the found document.


**Case 2: Updating fields via Import**
...
### Option 1: Setting up a new Table
...
- Choose Pick a column and select touristArea.
